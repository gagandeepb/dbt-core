...
class TestModels(DBTIntegrationTest):
    ...
    def test_postgres_pp_models(self):
        # create the models dir for the test in the `self.test_root_dir`
        # directory after everything else is symlinked. We can copy to and
        # delete files in this directory without messing with the source code
        os.mkdir(os.path.join(self.test_root_dir, 'models-test'))
        self.copy_file('models-a/model_one.sql', 'models-test/model_one.sql')
        # initial run
        self.run_dbt(['clean'])
        ...
        # add a model file
        # shutil.copyfile('extra-files/model_two.sql', 'models-test/model_two.sql')
        self.copy_file('extra-files/model_two.sql', 'models-test/model_two.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 2)

        # add a schema file
        # shutil.copyfile('extra-files/models-schema1.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/models-schema1.yml', 'models-test/schema.yml')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 2)
        ...
        self.assertEqual(model_one_node.patch_path, 'test://' + normalize('models-test/schema.yml'))

        # add a model and a schema file (with a test) at the same time
        # shutil.copyfile('extra-files/models-schema2.yml', 'models-test/schema.yml')
        # shutil.copyfile('extra-files/model_three.sql', 'models-test/model_three.sql')
        self.copy_file('extra-files/models-schema2.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/model_three.sql', 'models-test/model_three.sql')
        ...
        model_3_file_id = 'test://' + normalize('models-test/model_three.sql')
        ...
        schema_file_id = 'test://' + normalize('models-test/schema.yml')
        ...
        # shutil.copyfile('extra-files/model_three_modified.sql', 'models-test/model_three.sql')
        self.copy_file('extra-files/model_three_modified.sql', 'models-test/model_three.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        manifest = get_manifest()
        ...
        # shutil.copyfile('extra-files/models-schema2b.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/models-schema2b.yml', 'models-test/schema.yml')
        results = self.run_dbt(["--partial-parse", "test"], expect_pass=False)
        manifest = get_manifest()
        schema_file_id = 'test://' + normalize('models-test/schema.yml')
        schema_file = manifest.files[schema_file_id]
        ...
        # shutil.copyfile('extra-files/models-schema1.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/models-schema1.yml', 'models-test/schema.yml')
        os.remove(normalize('models-test/model_three.sql'))
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 2)
        ...
        # shutil.copyfile('extra-files/model_three.sql', 'models-test/model_three.sql')
        self.copy_file('extra-files/model_three.sql', 'models-test/model_three.sql')
        os.remove(normalize('models-test/schema.yml'))
        results = self.run_dbt(["--partial-parse", "run"])
        ...
        self.copy_file('extra-files/models-schema2.yml', 'models-test/schema.yml')
        # os.remove(normalize('models-test/model_three.sql'))
        self.rm_file('models-test/model_three.sql')
        ...
        # shutil.copyfile('extra-files/model_four1.sql', 'models-test/model_four.sql')
        self.copy_file('extra-files/model_four1.sql', 'models-test/model_four.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        ...
        # os.remove(normalize('models-test/model_three.sql'))
        self.rm_file('models-test/model_three.sql')
        # shutil.copyfile('extra-files/model_four2.sql', 'models-test/model_four.sql')
        self.copy_file('extra-files/model_four2.sql', 'models-test/model_four.sql')
        # shutil.copyfile('extra-files/models-schema1.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/models-schema1.yml', 'models-test/schema.yml')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)
        ...
        # shutil.copyfile('extra-files/models-schema2.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/models-schema2.yml', 'models-test/schema.yml')
        # os.remove(normalize('models-test/model_four.sql'))
        self.rm_file('models-test/model_four.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)

        # Add a macro
        # shutil.copyfile('extra-files/my_macro.sql', 'macros/my_macro.sql')
        self.copy_file('extra-files/my_macro.sql', 'macros/my_macro.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)
        manifest = get_manifest()
        ...
        self.copy_file('extra-files/my_macro2.sql', 'macros/my_macro.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)

        # Add a macro patch
        # shutil.copyfile('extra-files/models-schema3.yml', 'models-test/schema.yml')
        self.copy_file('extra-files/models-schema3.yml', 'models-test/schema.yml')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)
        ...
        self.copy_file('extra-files/macros.yml', 'macros/macros.yml')
        results = self.run_dbt(["--partial-parse", "run"])
        ...
        # shutil.copyfile('extra-files/empty_schema.yml', 'models-test/eschema.yml')
        self.copy_file('extra-files/empty_schema.yml', 'models-test/eschema.yml')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)

        # Add version to empty schema file
        # shutil.copyfile('extra-files/empty_schema_with_version.yml', 'models-test/eschema.yml')
        self.copy_file('extra-files/empty_schema_with_version.yml', 'models-test/eschema.yml')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 3)

        # Disable model_three
        # shutil.copyfile('extra-files/model_three_disabled.sql', 'models-test/model_three.sql')
        self.copy_file('extra-files/model_three_disabled.sql', 'models-test/model_three.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 2)
        manifest = get_manifest()
        model_id = 'model.test.model_three'
        ...
        # shutil.copyfile('extra-files/model_three_disabled2.sql', 'models-test/model_three.sql')
        self.copy_file('extra-files/model_three_disabled2.sql', 'models-test/model_three.sql')
        results = self.run_dbt(["--partial-parse", "run"])
        self.assertEqual(len(results), 2)
        manifest = get_manifest()
        model_id = 'model.test.model_three'
